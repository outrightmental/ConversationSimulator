...
| **Owner** | Platform team |
| **Mitigation** | macOS: Gatekeeper notarisation is a hard requirement before public release (Stage 4). Windows: SmartScreen Extended Validation code signing is required. Both are tracked as release gates in `docs/STEAM_ROADMAP.md`. Certificates and signing infrastructure must be in place before the private beta depot is built. CI enforcement: the `build-desktop` job in `.github/workflows/release.yml` fails early when `MACOS_CODESIGN_CERT_BASE64`, `APPLE_ID`, or the Windows KMS signing secrets (`GCP_SA_KEY_JSON` + `GCP_KMS_KEY` + `WINDOWS_CODESIGN_CERT`) are absent on a `v*` tag, preventing an unsigned artifact from being published. The macOS signing step asserts all five Hardened Runtime entitlements and runs `spctl --assess` + `xcrun stapler validate` — any SKIP or FAIL on a `v*` tag causes the job to exit non-zero. |
| **Release-blocking** | YES |
| **Status** | MITIGATED |
...
| **Owner** | Platform team |
| **Mitigation** | Three-part fix, all three shipped for Windows in v0.2.8. (0) **The SDK is now actually in the shipped build.** Every Steam depot before v0.2.8 was built without `--features steam`, so `SteamAPI_Init` never ran and the chord forwarder was inert; the release workflow now passes the feature for the Windows Steam build and ships Valve-signed `steam_api64.dll` in the depot (Valve's Sep 12 2026 review of BuildID 24431329: "The Steam Overlay doesn't appear when pressing Shift+Tab"). (1) **Hotkey forwarding (all platforms):** `useSteamOverlay` forwards Shift+Tab to `steam_activate_overlay` and F12 to `steam_trigger_screenshot`. (2) **Decoy compositing surface (Windows):** `apps/desktop/src-tauri/vendor/tauri-plugin-steam-overlay-surface` (MIT, PSG Studios; vendored with panic-hardening for our `panic = "abort"` profile) creates a transparent, click-through owned window with a wgpu swapchain presenting empty frames at vsync so Steam has a surface to composite into; `GameOverlayActivated` is forwarded from a new Steamworks callback-pump thread, and F12 uses hooked screenshots (`HookScreenshots` + live `PrintWindow` capture) because the decoy's backbuffer never contains the game. `steam_overlay_status` exposes `{ overlay_enabled, surface_active, surface_error }` for the G3-03 readout. The G3-03 pass criterion in `docs/steam-mvp-scope.md` and the QA step in `docs/QA_STEAM_PLATFORM_MATRIX.md` require the overlay to **visibly composite over the app** on Windows, so "no crash" can no longer be recorded as a pass. **Still open:** verification on real Steam-launched Windows hardware across GPUs (the vendored code was verified upstream on one machine class; hybrid-GPU laptops least tested), and the macOS/Linux legs (SDK library bundling in the .app/AppImage plus overlay-over-WebKit, both unsolved). Known limitation: after alt-tab the forwarder is deaf until the page is clicked once — do **not** attempt a `webview.set_focus()` workaround (it kills Shift+Tab entirely). Full write-up: [STEAM_INTEGRATION.md § Steam overlay (Windows WebView2 caveat)](../docs/STEAM_INTEGRATION.md#steam-overlay-windows-webview2-caveat). |
| **Release-blocking** | YES |
| **Status** | IN PROGRESS |

### SP-07 — PRC Interim Measures on AI Anthropomorphic Interaction Services

| | |
|---|---|
| **Area** | Platform / Legal |
| **Risk** | Valve's build review (Sep 12 2026, ticket HT-V9M8-5C9H-6YGN) failed the build pending confirmation that the app complies with the PRC *Interim Measures for the Administration of AI Anthropomorphic Interaction Services* (CAC Order No. 21, in force 2026-07-15), citing the SDA's compliance-with-applicable-law warranty and noting that compliance "is required for games with AI anthropomorphic elements to be distributed in China". If the Measures applied, several provider obligations (user registration with age/guardian data, minor mode, provincial security-assessment filing, algorithm filing) are structurally incompatible with a no-account, offline, open-source app. |
| **Owner** | Outright Mental (publishing) |
| **Mitigation** | Assessment in [`PRC_AI_ANTHROPOMORPHIC_INTERACTION_MEASURES.md`](PRC_AI_ANTHROPOMORPHIC_INTERACTION_MEASURES.md). Position: the app is a *learning and education* practice tool — a category Article 2 excludes by name — and is not a "sustained emotional interaction" (companionship) service; the safety policy states it is not an AI companion and prohibits the relevant content. User-protection provisions the Measures care about are met in substance (runtime content policy, self-harm crisis redirect, local-only data with delete/export, one-click exit). **Decision 2026-09-16:** state that position to Valve (reply sent on HT-V9M8-5C9H-6YGN) *and* have the Steam package restricted from purchase in mainland China so the "provided to the PRC public" element is not met at all — requested from Valve via Steamworks Support ticket HT-VN34-VN5B-6VMB (purchase restrictions are applied by Valve, not self-service); verifiable by Valve, reversible later. Cheap alignment follow-ups tracked in the assessment (in-session AI disclosure line; 2-hour continuous-use reminder; stated complaint-response time). Not legal advice; confirm with PRC counsel before any deliberate China release. |
| **Release-blocking** | YES — Valve's review is gated on the reply |
| **Status** | IN PROGRESS |
...
| SP-07 | Platform / Legal | YES | IN PROGRESS |
