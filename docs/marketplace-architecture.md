...
- A signature verification step in `loadPack()` in
  `packages/pack-loader/src/loader.ts`.
...
The pack manifest schema (`schemas/pack.schema.json`) would need the following
additions for marketplace-distributed packs:
...
- [`packages/pack-loader/src/loader.ts`](../packages/pack-loader/src/loader.ts) — pack loading and security scanning; see [Pack signing](#pack-signing) above for additions
- [`ROADMAP.md`](../ROADMAP.md) — UGC ecosystem section lists in-app pack browser and pack signing as future work outside MVP
