---
title: "Local models"
description: "How to download, verify, and manage the local language models that power NPC dialogue, including Ollama and custom GGUF options."
sidebar:
  order: 1
---
...
All inference happens on your computer. No prompts, transcripts, or model outputs are sent to any external server during play. See [privacy.md](/trust/privacy/) for the full data-handling policy and [network-security.md](/trust/network-security/) for how the local-only guarantee is enforced.
...
The model manager shows curated models from the registry ([`model-registry/registry.yaml`](https://github.com/outrightmental/ConversationSimulator/blob/main/model-registry/registry.yaml)). For each model it displays:
...
**Partial VRAM fit:** if you have less VRAM than the minimum, the model can still load with a reduced number of GPU-offloaded layers. Inference will be slower but may be acceptable. See [troubleshooting](/start/troubleshooting/#low-vram-or-slow-inference).
...
Compare the output against the checksum in [`model-registry/registry.yaml`](https://github.com/outrightmental/ConversationSimulator/blob/main/model-registry/registry.yaml).
...
- [Troubleshooting](/start/troubleshooting/) — model load failures, slow inference, low VRAM
