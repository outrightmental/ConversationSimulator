---
title: "Release notes"
description: "Release notes for Conversation Simulator, covering fixes, new features, downloads, system requirements, and known limitations for each release."
sidebar:
  order: 2
---
...
See [docs/install.md](/start/install/) for full prerequisites and
[docs/platform-notes.md](/play/platform-notes/) for platform-specific notes.
...
- **Content filtering.** The simulator applies keyword pre-checks and output
  ...
  [docs/safety-policy.md](/trust/safety-policy/) for details.
...
- **Voice input and output** are implemented but require separate manual runtime
  setup: whisper.cpp for STT
  ([`runtimes/whisper_cpp/`](https://github.com/outrightmental/ConversationSimulator/tree/main/runtimes/whisper_cpp))
  and a local Kokoro server for TTS
  ([`runtimes/kokoro/`](https://github.com/outrightmental/ConversationSimulator/tree/main/runtimes/kokoro)).
  See the runtime READMEs for setup steps.
...
[docs/post-alpha-issues.md](/dev/post-alpha-issues/).
...
See [docs/install.md](/start/install/) and
[docs/platform-notes.md](/play/platform-notes/) for full prerequisites and
