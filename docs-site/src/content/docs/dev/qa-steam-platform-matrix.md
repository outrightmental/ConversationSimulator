---
title: "QA platform matrix"
description: "Manual and automated test coverage required for a credible all-platform Steam release of Conversation Simulator, including hardware tiers, test cases, performance thresholds, and QA sign-off templates."
sidebar:
  order: 35
---
...
> This is the QA companion to [steam-mvp-scope.md](/dev/steam-mvp-scope/) (feature
> gates) and [release-checklist.md](/dev/release-checklist/) (release smoke steps).
...
[steam-mvp-scope.md](/dev/steam-mvp-scope/) and/or steps in
[release-checklist.md](/dev/release-checklist/). **Automated** means the case is
...
| 11.12 | Open and close Steam overlay (Shift+Tab) during a session | Overlay **is visibly drawn over the app** and closes; controller focus resumes correctly in the app. **Windows:** the overlay is a silent no-op unless the app forwards the chord *and* provides a decoy compositing surface — Steam reports the player In-Game / `is_overlay_enabled()` true / `GameOverlayActivated` fired even when nothing is drawn, so treat an invisible overlay as FAIL. Also press F12: a screenshot of the live app (not a black/stale frame) must appear in the Steam screenshot library. Record GPU, Windows build, the `steam_overlay_status` readout (`overlay_enabled` / `surface_active` / `surface_error`), and whether the surface came up (see [Steam integration § Steam overlay (Windows WebView2 caveat)](/dev/steam-integration/#steam-overlay-windows-webview2-caveat)). | Manual |
...
[`docs/STEAM_BETA_VERIFICATION_REPORT.md`](/dev/steam-beta-verification/).
...
- [ ] Release-blocking risks in [`STEAM_COMPLIANCE_AND_RISK_REGISTER.md`](https://github.com/outrightmental/ConversationSimulator/blob/main/publishing/STEAM_COMPLIANCE_AND_RISK_REGISTER.md) show MITIGATED, ACCEPTED, or DEFERRED (G3-04).
...
- [steam-mvp-scope.md](/dev/steam-mvp-scope/) — feature requirements and pass/fail gates (G2–G4)
- [STEAM_ROADMAP.md](/dev/steam-roadmap/) — release principles, release train, Steam Deck verification checklist
- [STEAM_BETA_VERIFICATION_REPORT.md](/dev/steam-beta-verification/) — aggregate signed-off beta verification report template (all four platforms)
- [release-checklist.md](/dev/release-checklist/) — Parts A–D platform smoke matrix; Parts E (Windows), G (macOS), H (Linux/SteamOS), I (Steam Deck), J (sign-off) for Steam beta verification
- [performance.md](/play/performance/) — hardware tier definitions and latency guidance
- [platform-notes.md](/play/platform-notes/) — platform-specific build and install details
- [linux-steamos-requirements.md](/play/linux-steamos/) — glibc requirements, SteamOS hardware profile, AppImage behavior
- [privacy.md](/trust/privacy/) — local-first promise and data handling
- [network-security.md](/trust/network-security/) — runtime network enforcement
- [voice-smoke-tests.md](/dev/voice-smoke-tests/) — STT/TTS smoke test procedures
- [publishing/STEAM_COMPLIANCE_AND_RISK_REGISTER.md](https://github.com/outrightmental/ConversationSimulator/blob/main/publishing/STEAM_COMPLIANCE_AND_RISK_REGISTER.md) — compliance checklists and risk register
