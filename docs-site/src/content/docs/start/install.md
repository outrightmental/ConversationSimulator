---
title: "Installation"
description: "How to install Conversation Simulator, either from source as a developer or from the pre-built desktop app releases."
sidebar:
  order: 1
---
...
| GPU VRAM | 0 GB (CPU fallback available) | 4 GB+ for starter model; see [local-models.md](/play/local-models/) |
...
See [local-models.md](/play/local-models/) for model recommendations by hardware.
...
3. Data created in a newer beta is forward-compatible with older betas at the
   same schema version (see [schemas/VERSIONING.md](/reference/schema-versioning/)).
...
> WebView2, audio permissions), see [platform-notes.md](/play/platform-notes/).
